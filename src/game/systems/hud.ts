import type { HudState } from '../types/gameplay';
import {
    GAME_HEIGHT,
    NODE_CENTER_X,
    NODE_CENTER_Y
} from '../config/gameplay';

export type HudElements = {
    hudText: Phaser.GameObjects.Text;
    stateText: Phaser.GameObjects.Text;
};

export const createHud = (scene: Phaser.Scene): HudElements => {
    const hudText = scene.add.text(20, 18, '', {
        fontFamily: 'Arial',
        fontSize: 18,
        color: '#e2e8f0',
        lineSpacing: 8
    }).setDepth(200);

    scene.add.text(20, GAME_HEIGHT - 48, 'WASD/Frecce: movimento  |  Fuoco automatico  |  R: restart dopo morte', {
        fontFamily: 'Arial',
        fontSize: 16,
        color: '#94a3b8'
    }).setDepth(200);

    const stateText = scene.add.text(NODE_CENTER_X, NODE_CENTER_Y - 12, '', {
        fontFamily: 'Arial Black',
        fontSize: 40,
        color: '#ffffff',
        stroke: '#0f172a',
        strokeThickness: 8,
        align: 'center'
    }).setOrigin(0.5).setDepth(250);

    return {
        hudText,
        stateText
    };
};

export const updateHud = (hudText: Phaser.GameObjects.Text, state: HudState) => {
    const xpClamped = Math.min(state.xp, state.xpToNext);

    hudText.setText([
        `HP ${state.hp}/${state.maxHp}`,
        `XP ${xpClamped}/${state.xpToNext}  Livello ${state.level}`,
        `Monete run ${state.coins}`,
        `Wave ${state.wave}`,
        `Nemici ${state.enemyCount}`
    ]);
};
